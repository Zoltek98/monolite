...
const InvestmentsSection: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [resAssets, resHistory] = await Promise.all([
        axios.get<IAsset[]>('http://api.casa-boschetto.com.com/api/assets'),
        axios.get<IPortfolioHistory[]>('http://api.casa-boschetto.com.com/api/portfolio-history')
      ]);
  ...
  const handleUpdateAsset = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.put(`http://api.casa-boschetto.com.com/api/assets/${selectedAsset.id}`, {
